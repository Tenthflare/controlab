...
def evaluate_checkpoint(eval_config) -> None:
    ...
    test_set = spec.make_test_set(buckets=test_buckets,
        ...
        for bucket in test_buckets:
            for set_id, set_values in enumerate(test_set[bucket]):
                for episode in range(num_episode):
                    ...
                    episode_seed = eval_base_seed + ((bucket_idx * n_per_bucket + set_id) * num_episode + episode)
                    obs, info = env.reset(seed=episode_seed, options={"sampled_values": set_values})  # inject frozen dynamics
                    ...
                    for _ in range(horizon):
                        ...
                        if done: break
                    rows.append({"alpha": run["alpha"], "seed": run["seed"],
                                 "bucket": bucket, "set_id": set_id, "episode": episode,
                                 "episode_reward": ep_reward, "survived": bool(truncated and not terminated)})

    pd.DataFrame(rows).to_parquet(eval_output_path)


if __name__ == "__main__":
    eval_config = json.loads((REPO_ROOT / "configs" / "eval_config.json").read_text())
    evaluate_checkpoint(eval_config)
